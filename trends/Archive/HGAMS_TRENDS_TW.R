...
#-----------------------------------------------------------------------------
# STEP 3: FIT TWEEDIE MODELS (S vs GS) AND COMPARE AIC
#-----------------------------------------------------------------------------
# We will fit both models using the Tweedie family.

# MODEL 1: The 'S' Model (Separate smooths for each species)
gam_model_S <- gam(
  abundance ~ species + s(year, by = species, bs = "tp"),
  data = community_ts_subset,
  family = tw(a = 1.2, b = 1.8), # Using Tweedie family
  method = "REML"
)
...
print(paste("AIC for Tweedie S Model:", round(aic_S, 2)))
print(paste("AIC for Tweedie GS Model:", round(aic_GS, 2)))
